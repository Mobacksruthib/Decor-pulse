...
class MyCart extends PureComponent {
  ...
  getCart() {
    ...
    let params = ["/" + dealerCode + "_01/getcartfabric/" + this.state.cartId];
      ...
      Promise.all([api.postData("MODIFYCART", raw, access_token)])
        .then((result2) => {
          try {
            ...
            if(data2.isCartInValidOrExpired=='false'){
    ...
    Promise.all([api.postData("CARTSUBMIT", raw, access_token)])
      .then((result2) => {
        try {
          ...
          if ((data2.erpOrderId !== undefined) && (data2.erpOrderId !=''))  {
            //this.state.successmsg =   +"\nSuccessFull\n\n"+"Order Id : " + data2.erpOrderId +"\n\nOrder Amount: " +data2.erpOrderTotal ;
            this.state.successmsg =   "\n\nOrder SuccessFul\n\n"+"Order Id : " + data2.erpOrderId +"\n\nOrder Amount: " +data2.erpOrderTotal;
            this.state.newItems =[];
            this.props.cartDetails.cartId=0;
            this.state.cartId=0;
            this.state.isVisible = true;
            /*this.toast.show(msg,4500,
              () => {
  ...
  renderTopBanner = () => {
    return (
      <View style={styles.topBanner}>
        <Text style={styles.topBannerText}>MyCart</Text>
      </View>
    );
  };
    ...
    if (this.state.emptyCart === false) {
      return (
        <SafeAreaView style={styles.container}>
          ...
          >
            <Text style={{ fontSize: 18, fontWeight: "bold" }}>PaymentTerms</Text>
            <TouchableOpacity
                onPress={() => {
            ...
            <Modal            
              ...
              {/*All views of Modal*/}  
                <View style = {styles.modal}>  
                  <Text style = {styles.text}>{this.state.successmsg}</Text>  
                  <Button title="Close" color="red" onPress = {() => {  
                      this.setState({ isVisible:!this.state.isVisible, cartItems:[] });             
                      navigation.navigate("MyOrders");
                    }}/>  
                </View>  
            </Modal>  
             
          {(this.state.emptyCart === false) ? this.renderCart() : null}
          {this.renderFooter()}
          <Toast
